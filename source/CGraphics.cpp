...
*******************************************************************/
CGraphics::CGraphics(HWND hWnd, int ScreenWidth, int ScreenHeight, int RefreshRate, D3DFORMAT Format, int Adapter, bool FullScreen)
{
	m_hWnd 			= hWnd;
...
bool CGraphics::IsDisplayModeSupported()
{
	...
	if(m_d3d)
	{
		AdapterModeCount = m_d3d->GetAdapterModeCount(m_adapter, m_format);
		pLog->Log("Info: IDirect3D9::GetAdapterModeCount() Count. ", AdapterModeCount);

		for(int Loop = 0; Loop < AdapterModeCount; Loop++)
		{
			if(FAILED(m_d3d->EnumAdapterModes(m_adapter, m_format, Loop, &Mode)))
			{
				pLog->Log("Error: IDirect3D9::EnumAdapterModes() Failed.");
				return false;
			}

			pLog->Log("Info: IDirect3D9::EnumAdapterModes() Testing Width.       ", Mode.Width);
			pLog->Log("Info: IDirect3D9::EnumAdapterModes() Testing Height.      ", Mode.Height);
			pLog->Log("Info: IDirect3D9::EnumAdapterModes() Testing RefreshRate. ", Mode.RefreshRate);

			if(m_screenWidth == Mode.Width & m_screenHeight == Mode.Height & m_refreshRate == Mode.RefreshRate)
			{
				return true;
			}
		}
...
bool CGraphics::InitializeDevice()
{
	...
	m_d3dpp.FullScreen_RefreshRateInHz	= m_fullScreen ? m_refreshRate : 0;
	m_d3dpp.EnableAutoDepthStencil		= true;
	m_d3dpp.AutoDepthStencilFormat		= FindDepthStencilFormat(D3DADAPTER_DEFAULT, CurrentMode, D3DDEVTYPE_HAL);
	
	if(FAILED(m_d3d->CreateDevice(m_adapter, D3DDEVTYPE_HAL, m_hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &m_d3dpp, &m_d3ddev)))
	{
