...
void CGameStatePlay1::Activate(CGameData &data, CConfigData &cfg, CGraphics &con){
  ...
  m_addTarget = ADD_TILE; // 0 = tiles, 1 = things, 2 = sounds
  ...
  if(m_addTarget == ADD_TILE)
    m_iconTotalTiles = m_tileLevel.GetTileDefSize(); 
  else if(m_addTarget == ADD_OBJECT)
    m_iconTotalTiles = m_objectManager.GetObjectDefinitionSize();

  m_keyMode = 0;//move orthogoanlly
  m_scale = 1.0;
  m_currentGroupID = 0;
  m_currentObjectGroupID = 0;
  m_lastObjectGroupID = 0;
  m_lastTileGroupID = 0;
...
//  timeDifference *= data.m_timeScale; //multiply by 1 or 2
  ...
  //add tile
  if(m_addTarget == ADD_TILE && data.m_bShowMap == false && data.m_bLeftMouseDown == true && m_mouse.MouseY() > 56 && m_mouse.MouseY() < 690){
    m_tileLevel.AddTile(data.m_brushSize);
  }
  //add tile via map
  if(m_addTarget == ADD_TILE && data.m_bShowMap == true && data.m_bLeftMouseDown == true && m_mouse.MouseY() > 56 && m_mouse.MouseY() < 690){
  ...
  //***************************************************************
    ...
    //scroll list right
      ...
      if(m_addTarget == ADD_TILE){
        ...
        if(m_iconSelectPosition < -70 * m_objectManager.GetGroupSize())//lower limit
          m_iconSelectPosition = -70 * m_objectManager.GetGroupSize();
      }      
      ...
      if(m_addTarget == ADD_TILE){
        ...
        if(iconPosX > 0 && iconPosX < m_objectManager.GetGroupSize() + 1){// .GetObjectDefinitionSize() + 1){// .GetObjectDefinitionSize() + 1){
          m_iconTileSelectIndex = iconPosX - 1; 
          m_objectManager.SetGroupTileID(m_iconTileSelectIndex);
        }
      }
  ...
  //**************************************************************
  else if(data.m_bLeftMouseDown == true && m_mouse.MouseY() < 33 + data.m_windowedYOffset && fileTime > 0.4){
    fileTime = 0;
    if(m_mouse.MouseX() > 221 && m_mouse.MouseX() < 233){ //increment file name
    ...
    else if((keyDown(VK_LSHIFT) == true || keyDown(VK_RSHIFT)) && m_mouse.MouseX() > 6 && m_mouse.MouseX() < 56){// open file
      m_tileLevel.LoadLevelFile(cfg.RootPath + m_filenames[m_fileLevel]);
      m_objectManager.LoadLevelFile(cfg.RootPath + m_filenames[m_fileLevel], con);
    }
    else if((keyDown(VK_LSHIFT) == true || keyDown(VK_RSHIFT)) && m_mouse.MouseX() > 59 && m_mouse.MouseX() < 107){// save file
    ...
    else if((keyDown(VK_LSHIFT) == true || keyDown(VK_RSHIFT)) && m_mouse.MouseX() > 238 && m_mouse.MouseX() < 296){// clear level
      m_tileLevel.ClearTiles();  
    ...
    else if((keyDown(VK_LSHIFT) == true || keyDown(VK_RSHIFT)) && m_mouse.MouseX() > 994 && m_mouse.MouseX() < 1018){//exit
      //m_event = EVENT_GO_MAIN;
      m_event = EVENT_GO_QUIT;
    }
    ...
    else if(m_mouse.MouseX() > 416 && m_mouse.MouseX() < 422){//single brush
      data.m_brushSize = 1; 
    }
    else if(m_mouse.MouseX() > 422 && m_mouse.MouseX() < 440){//large brush
      data.m_brushSize = 2; 
    }    
      ...
      if(m_addTarget == ADD_TILE){
        ...
        m_currentGroupID--;
        if(m_currentGroupID < 0)
          m_currentGroupID = m_objectManager.GetObjectDefinitionSize() - 1;
        m_objectManager.SetGroupNumber(m_currentGroupID);
        m_iconTileSelectIndex = 0;
        m_iconSelectPosition = 0;
      }
    }
      ...
      if(m_addTarget == ADD_TILE){
        ...
        m_currentGroupID++;      
        if(m_currentGroupID > m_objectManager.GetObjectDefinitionSize() - 1)
          m_currentGroupID = 0;
        m_objectManager.SetGroupNumber(m_currentGroupID);
        m_iconTileSelectIndex = 0;
        m_iconSelectPosition = 0;
    ...
    else if(m_mouse.MouseX() > 566 && m_mouse.MouseX() < 594){//add tiles
      m_lastObjectGroupID = m_currentGroupID;
      m_currentGroupID = m_lastTileGroupID;
      m_iconSelectPosition = 0;
      m_tileLevel.SetGroupNumber(m_currentGroupID);
      m_addTarget = ADD_TILE;
      m_iconTileSelectIndex = 0;
      m_iconSelectPosition = 0;
    }
    else if(m_mouse.MouseX() > 594 && m_mouse.MouseX() < 623){//add objects
      m_lastTileGroupID = m_currentGroupID;
      m_currentGroupID = m_lastObjectGroupID;
      m_iconSelectPosition = 0;
      m_objectManager.SetGroupNumber(m_currentGroupID);
      m_addTarget = ADD_OBJECT;
      m_iconTileSelectIndex = 0;
      m_iconSelectPosition = 0;
    }
  }
  ...
  //remove tile
  //*******************************************************
  if(m_addTarget == ADD_TILE && data.m_bShowMap == false && data.m_bRightMouseDown == true && m_mouse.MouseY() > 56 && m_mouse.MouseY() < 690){
    m_tileLevel.RemoveTile();
  }
  else if(m_addTarget == ADD_TILE && data.m_bShowMap == true && data.m_bRightMouseDown == true && m_mouse.MouseY() > 56 && m_mouse.MouseY() < 690){
    m_tileLevel.RemoveTileViaMap(m_mouse.MouseX(), m_mouse.MouseY(), data.m_brushSize);
  }
  else if(m_addTarget == ADD_OBJECT && data.m_bShowMap == false && data.m_bRightMouseDown == true && m_mouse.MouseY() > 56 && m_mouse.MouseY() < 690){
    m_objectManager.RemoveObject(data.m_posX - m_mouse.MouseX(),data.m_posY - m_mouse.MouseY());
  }
...
int CGameStatePlay1::Render(CGraphics &con,  CGameData &data, CConfigData &cfg){
  ...
  //tile selection interface for the tiles
  //***********************************************************
  ...
  //render black squares for the objects
  else if(m_addTarget == ADD_OBJECT){
    for(int i = 0; i < m_objectManager.GetGroupSize();++i){   
      gi = con.GetSprite(2002);  
      con.RenderSprite(gi, m_iconSelectPosition + 32 + (i * 70), 696, 255, 255, 255);//black square
      gi = con.GetSprite(m_objectManager.GetGroupObjectSprite(i)); 
      gi.scale = 0.33f;
        con.RenderSprite(gi, m_iconSelectPosition + 32 + (i * 70) + 34 - (gi.scale * gi.width/2), 696 + 34 - (gi.scale * gi.height/2), 255, 255, 255);//black square
    } 
  }
...
//  m_UI.RenderOutlines(con, data);
  ...
  con.Print("Group No.", m_currentGroupID, F_V16B, 628, 8, 255, 255, 255, 255);
  ...
  //draw outline around iso view
  if(m_tileLevel.GetViewMode() == 0)
    con.DrawRect(446, 5, 472, 27, 255, 255, 255);
  else
    con.DrawRect(476, 5, 507, 27, 255, 255, 255);

  //draw outline around target
  if(m_addTarget == ADD_TILE)
    con.DrawRect(566, 5, 594, 27, 255, 255, 255);
  else
    con.DrawRect(596, 5, 623, 27, 255, 255, 255);

  //draw outline around map
  if(data.m_bShowMap == true)
    con.DrawRect(366, 5, 410, 27, 255, 255, 255);

  //highlight brush size
  if(data.m_brushSize == 1)
    con.DrawRect(416, 5, 422, 15, 255, 255, 255);
  else
    con.DrawRect(424, 5, 440, 27, 255, 255, 255);

  //highlight grid
  if(m_tileLevel.IsDrawGrid() == true)
    con.DrawRect(965, 5, 989, 27, 255, 255, 255);
